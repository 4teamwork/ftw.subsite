...
class SubsiteView(BrowserView):
    ...
    def empty(self):
        dashboards = [getUtility(IPortletManager, name=name) for name in
                        ['ftw.subsite.front1', 'ftw.subsite.front2', 'ftw.subsite.front3', 'ftw.subsite.front4', 'ftw.subsite.front5', 'ftw.subsite.front6']]
    ...
    def hasPermissions(self):
        ...
        if not isAnon:
            hasPermission = member_tool.getAuthenticatedMember().has_permission(permissionToCheck, current_context)
        else:
            return False
